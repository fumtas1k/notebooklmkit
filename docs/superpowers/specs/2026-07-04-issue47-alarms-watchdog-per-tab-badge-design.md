...
なお、pendingCreate は単一スロットのため、1 分以内に複数タブを連続クリップすると、
先行タブの '…' はウォッチドッグ（後続クリックで上書き・再スケジュールされる）に
拾われず、そのタブに固着し得る（次回そのタブでクリップするまで自己回復しない）。
従来のグローバルバッジでは視覚的にマスクされていた挙動で、tabId 別スコープ化で
顕在化する。本質的な解消には per-tab キーの pendingCreate ＋ per-tab alarm が必要で、
複数タブ選択 UI 統合時にまとめて対応する（フォローアップ issue）。
