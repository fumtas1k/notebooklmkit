...
- **バッチ成功の意味論**: バッチ成功はダイアログが閉じたこと（＝コミット成功）しか観測しないため、
  全 URL を succeeded として報告する。NotebookLM 側が「形式は妥当だが到達不能」な1件を暗黙に取りこぼしても
  検知できない（逐次パスは1件ずつ失敗を掴めるので、これはバッチ化に伴う忠実度の低下）。到達性は投入前検証の
  対象外（`urlCounts` は形式 valid/invalid のみ）。
- **フォールバックのテストと実機の乖離**: pre-commit フォールバックのユニットテストは jsdom で add ボタンが
  常に効くため「復旧」して緑になるが、実機ではコミット前失敗時に URL 入力済みのソース追加ダイアログが
  モーダルで開いたまま残り、多くの場合フォールバックは即失敗→1件記録して停止（＝復旧しない）になり得る。
  テストが緑であることは実機の復旧を保証しない（e2e チェックリストは「復旧 OR 孤立ダイアログを残さず安全停止」と OR で記述済み）。
