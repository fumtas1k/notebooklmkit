...
2. **confirm 後の再検証のみ** — どんな経路の食い違いも削除直前に検出できるが、モーダル表示中に背後を操作できるという壊れたモーダル意味論（a11y 問題）が残る。
3. **両方**（採用） — フォーカストラップが報告経路の根本対策、再検証が取り消し不可操作に対する最終安全網。どちらも実装が小さく、防御の層が独立している。
...
if (ev.key === 'Tab') {
  ev.preventDefault()
  ev.stopPropagation()
  const els = Array.from(box.querySelectorAll<HTMLElement>('button:not([disabled]), input'))
...
- `document.activeElement` がダイアログ外にある場合（`idx === -1`）は先頭（Shift+Tab なら末尾）へ引き戻す。
