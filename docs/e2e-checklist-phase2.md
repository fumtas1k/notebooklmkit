...
- [ ] #51: 作成完了後、Studio の「音声解説」生成タイルが自動押下され「音声解説を生成しています…」表示になる（カスタマイズ chevron ではなくタイル本体が押されること）。
  - [ ] ソース解析中でタイルが未表示/無効な場合、present+enabled を待って押される（60s 以内。超過時は best-effort で諦め、作成自体は '✓' のまま）。実ポインタ列で押すため、タイル（div[role=button]）でも生成が始まること。
