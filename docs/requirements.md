...
- **クリック方式**: タイルは `div[role="button"]`（Angular Material）で、`el.click()`（合成クリック）では
  生成ハンドラが発火しない。実ポインタ列（pointerdown → mousedown → pointerup → mouseup → click、
  座標つき）を送る必要がある（`dom-utils.ts` の `pointerClick`。実 `<button>` フォールバックでも
  click が最後に発火するため安全）。`triggerAudioOverview` の click には `pointerClick` を配線する。
- **解析待ち**: ソース解析中はタイルが present+enabled にならないことがあるため、`triggerAudioOverview` の
  既定タイムアウトは 60s（best-effort・fire-and-forget なので長めでも無害。E2E で調整）。
