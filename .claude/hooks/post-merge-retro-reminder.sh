...
# 早期ゲート: 入力に "gh pr merge" という語句自体が含まれなければ、
# jq を一切呼ばずに即終了する（jq×5 回避）。安全側 no-op は維持
# ——語句を含むが実はマージでないケースは後段のセグメント解析で弾く。
case "$input" in
    *"gh pr merge"*) ;;
    *) exit 0 ;;
esac
...
#
# 移植性の注意: `sed` の置換文字列側 `\n` は GNU 拡張であり、BSD sed
# （macOS 標準 /usr/bin/sed）では改行に展開されない。そのため BSD/GNU
# 両対応で書く —— `&&` / `||` はリテラル改行を埋め込む `\<改行>` イディオム
# （`\'$'\n''` は「シェルでリテラル改行を作って sed に渡す」構文）で改行化し、
# 単一文字区切りの `;` / `|` は `tr` で改行化する（`||` を先に潰しておくため、
# tr の時点で残る `|` は単独パイプのみ）。
normalized="$(printf '%s' "$command_str" \
    | sed -E 's/&&/\'$'\n''/g; s/\|\|/\'$'\n''/g' \
    | tr ';|' '\n\n')"
